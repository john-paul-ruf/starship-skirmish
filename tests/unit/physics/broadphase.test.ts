...
describe('broadphase — trivial cases', () => {
  it('returns [] for 0 bodies', () => {
    expect(broadphase([])).toEqual([]);
  });

  it('returns [] for 1 body', () => {
    expect(broadphase([ship(1)])).toEqual([]);
  });
});

describe('broadphase — cell adjacency', () => {
  // radius 10 → cellSize 20 for every scene below.

  it('pairs bodies in the same cell', () => {
    const pairs = broadphase([ship(1, of(0, 0, 0)), ship(2, of(5, 0, 0))]);
  ...
  it('pairs bodies in adjacent cells (one cell apart)', () => {
    ...
    const pairs = broadphase([ship(1, of(0, 0, 0)), ship(2, of(25, 0, 0))]);
    expect(pairs).toEqual([{ a: 1, b: 2 }]);
  });

  it('does NOT pair bodies more than one cell apart', () => {
    // Body 2 sits three cells away — outside every neighborhood.
    const pairs = broadphase([ship(1, of(0, 0, 0)), ship(2, of(60, 0, 0))]);
...
describe('broadphase — canonical ordering', () => {
  it('emits pairs ascending by (a, b) regardless of input order', () => {
    ...
    expect(broadphase(scene)).toEqual([
      { a: 1, b: 2 },
      { a: 1, b: 3 },
      { a: 2, b: 3 },
    ]);
  ...
  it('is fully order-independent: shuffled input yields identical output', () => {
    ...
    for (const perm of permutations) expect(broadphase(perm)).toEqual(ref);
  });
...
describe('broadphase — cell size adapts to largest body', () => {
  it('uses 2·maxRadius so a giant body still finds neighbors it could touch', () => {
    // Body 1: radius 100 → cellSize 200. Body 2 sits 150 units away — same or neighbor cell.
    const pairs = broadphase([
      { kind: 'ship', id: 1, position: of(0, 0, 0), velocity: of(0, 0, 0), radius: 100, mass: 10 },
      { kind: 'ship', id: 2, position: of(150, 0, 0), velocity: of(0, 0, 0), radius: 10, mass: 1 },
    ]);
    expect(pairs).toEqual([{ a: 1, b: 2 }]);
  });
});
