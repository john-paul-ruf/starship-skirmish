...
| 03 | Rebuild Tactical Attack to the Mock Contract | M14, M19 | `./src/ui/screens/TacticalAttack.tsx`; `./src/ui/screens/tacticalAttack/Viewport.tsx`; `./src/ui/screens/tacticalAttack/FieldOverlay.tsx`; `./src/ui/screens/tacticalAttack/WeaponBench.tsx`; `./src/ui/screens/tacticalAttack/CalledShotPicker.tsx`; `./src/ui/screens/tacticalAttack/CommitBar.tsx`; `./src/ui/screens/tacticalAttack/model.ts`; `./tests/unit/ui/tacticalAttack/layout.test.ts`; `./tests/unit/ui/tacticalAttack/model.test.ts`; `./tests/e2e/tacticalAttack.spec.ts`; `./tests/e2e/tacticalAttack.spec.ts-snapshots/**` | done | 5/5 | 2026-08-28 | Rebuilt Tactical Attack to the mock's literal three-column frame — 288px all-fleet roster / fluid tactical stage with center-only combat log / 344px selected-shooter fire rail — replacing the false single-second-column stack. The rail renders ONE active shooter (D-TA-RAIL-SHOOTER) with authored weapon/chassis names; live overlays (all weapon range envelopes + labels, firing-solution lines with controller-derived % pills, AoE ring + friendly warning, boundary/legend/beat-HUD) render from live state; hit chance stays single-sourced through hitChanceFor; blind commit stays structural. Verified in a real browser (14/14 e2e) with a reviewed real-M13-render screenshot baseline. |
...
### SESSION-03 — done · 2026-08-28 · checkpoint 5/5 · Mu→Enso

**notes:** Rebuilt Tactical Attack to the mock's literal three-column frame — 288px all-fleet roster / fluid tactical stage with center-only combat log / 344px selected-shooter fire rail — replacing the false single-second-column stack. The rail renders ONE active shooter (D-TA-RAIL-SHOOTER) with authored weapon/chassis names; live overlays (all weapon range envelopes + labels, firing-solution lines with controller-derived % pills, AoE ring + friendly warning, boundary/legend/beat-HUD) render from live state; hit chance stays single-sourced through hitChanceFor; blind commit stays structural. Verified in a real browser (14/14 e2e) with a reviewed real-M13-render screenshot baseline.

**followUp:** SESSION-03 owns the full tactical-attack UI/test set. The committed screenshot baseline is chromium-darwin only; a CI run on linux will report a missing -chromium-linux snapshot — generate the linux baseline in CI (or scope the snapshot test to darwin) before enabling this spec on CI. Range-ring labels are placed at each ring's projected top edge; a very large-radius envelope can clip off the top of the viewport (overflow:hidden clips gracefully — the real perspective render keeps them on-screen at fleet zoom). The e2e real-render harness constructs a minimal MatchState (bodies/ships/fleetOf/guidances/debrisAge) — reuse it if a future session needs a real-render mount seam.

**layoutClasses (verbatim):** `["1920×1080","1280×720"]`

**a11yNotes (verbatim):** Three surfaces are labelled landmarks (aside 'Fleet roster and ship inspector', main center stage, aside 'Fire assignment'). Decorative solution/AoE SVG is aria-hidden; percentage/status conveyed as text pills (OUT OF RANGE, FRIENDLY IN AoE, ASSIGNED/HOLD/LAUNCH). Active-shooter/selected/out-of-range/destroyed-subsystem/AoE states carry text+glyph, never colour alone. FULL FIELD/RESTORE toggle is text + aria-pressed; Esc wired only while immersive; focus rings from shipped classes; no pointer-only action added; the overlay is pointer-events:none so it never steals canvas/roster clicks.
