// M14 UI — Tactical Movement arc plotter (S05 CP2).
//
// Numeric-primary arc entry (Gate 1 §2): bearing / pitch as `<input
// type="number">`, magnitude as a slider clamped to the ship's Δv budget. Every
// input drives the live ghost (the parent recomputes `controller.previewArc`
// and repaints the Viewport). A COAST toggle spends no Δv (keeps momentum).
//
...
import { clampMag, deltaVMag, type PlanDraft, type RosterShip } from './model.js';
...
export function ArcPlotter({ ship, draft, velocity, exiting, onPlot, onCoast }: ArcPlotterProps) {
  ...
  const spent = deltaVMag(draft, budget);
    ...
    <section class="tm-plotter panel-bd" data-testid="arc-plotter">
      ...
      <div class="tm-budget">
        <div class="tm-budget-row">
          ...
          <span class="grow" />
          <span class="mono-xs">
            SPENT <span class="c-hi">{round(spent)}</span> / {round(budget)}
          </span>
        </div>
        <Meter
          value={spent}
          max={budget}
          fill="dv"
          aria-label={`Delta-V spent ${round(spent)} of ${round(budget)}`}
        />
      ...
      <div class="tm-mag">
        <div class="tm-budget-row">
          ...
          <span class="grow" />
          <span class="stat-v c-amber">{round(clampMag(draft.magnitude, budget))} Δv</span>
        </div>
        <input
          class="tm-range"
          type="range"
          ...
          max={budget}
          step={1}
          value={coasting ? 0 : clampMag(draft.magnitude, budget)}
          disabled={!ship.engineAlive}
        ...
        <div class="tm-range-scale mono-xs">
          <span>0 · COAST</span>
          <span class="c-hi">{round(budget)} · MAX</span>
        </div>
      </div>
